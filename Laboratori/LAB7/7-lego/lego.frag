...
void main()
{
    if (isTop > 0) fragColor = gfrontColor * texture(colorMap, gtexCoord);// * normalize(gnormal).z;
    else fragColor = gfrontColor;// * normalize(gnormal).z;
}*/
...
void main() {
  ...
  if (isTop>0) fragColor=C*TC;//*normalize(gnormal).z;
